...
Table of Contents:
    Step 1. Create the shapes (random OR fixed trajectory/speed)
...
`options` lets you configure the demo without editing this file:
    ...
    minSpeed             (default 2)     pixels moved per frame (slowest)
    maxSpeed             (default 6)     pixels moved per frame (fastest)
    useRandomTrajectory  (default true)  false = every shape moves with the
                                         exact same fixed trajectory
                                         (fixedVx/fixedVy) instead of a random
                                         speed + direction
    fixedVx, fixedVy     (default 3, 2)  only used when useRandomTrajectory
                                         is false
*/
function createCanvasAnimation(ctx, canvasWidth, canvasHeight, options) {
    ...
    var minSpeed = options.minSpeed || 2;
    var maxSpeed = options.maxSpeed || 6;
    var useRandomTrajectory = options.useRandomTrajectory !== undefined
        ? options.useRandomTrajectory
        : true;
    var fixedVx = options.fixedVx || 3;
    var fixedVy = options.fixedVy || 2;
    ...
    for (var i = 0; i < numShapes; i++) {
        ...
        if (useRandomTrajectory) {
            // Pick a completely random direction (an angle from 0 to 360
            // degrees, in radians) and a random speed within our range.
            // Splitting a speed into vx/vy this way with sin/cos is the
            // standard way to turn "how fast" + "which direction" into the
            // horizontal/vertical velocity components we can just add to
            // x/y every frame.
            ...
            // Fixed trajectory: every shape moves the exact same way.
            vx = fixedVx;
            vy = fixedVy;
        }
